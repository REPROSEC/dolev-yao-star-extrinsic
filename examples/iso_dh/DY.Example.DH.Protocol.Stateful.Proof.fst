...
let dh_session_pred: local_state_predicate dh_session = {
  pred = (fun tr prin sess_id st ->
    ...
    | InitiatorSendMsg3 bob gx gy k -> (
      ...
      (is_corrupt tr (long_signature_key_label bob) \/ is_corrupt tr (ephemeral_dh_key_label alice sess_id) \/
    ...
    | ResponderReceivedMsg3 alice gx gy k -> (
      ...
      (is_corrupt tr (long_signature_key_label alice) \/ is_corrupt tr (ephemeral_dh_key_label bob sess_id) \/
...
let send_msg2_proof tr global_sess_id bob bob_si =
  ...
  | (Some (ResponderSentMsg2 alice gx gy y), tr) -> (
    ...
    | (Some sk_b, tr) -> (
      let (n_sig, tr) = mk_rand SigNonce (long_signature_key_label bob) 32 tr in
    ...
    | (Some msg_bytes, tr) -> (
      ...
      | (Some pk_b, tr) -> (
        ...
        | Some res -> (
          ...
          let bob_not_corrupt = (~(is_corrupt tr (long_signature_key_label bob))) in
...
let send_msg3_proof tr global_sess_id alice alice_si bob =
  ...
  | (Some (InitiatorSendMsg3 bob gx gy k), tr') -> (
    ...
    | (Some sk_a, tr') -> (
      let (n_sig, tr') = mk_rand SigNonce (long_signature_key_label alice) 32 tr' in
      ...
      | (Some pk_a, tr) -> (
          ...
          | Some res -> (
            ...
            assert(exists x. gx == dh_pk x /\ event_triggered tr alice (Initiate2 alice bob gx gy (dh x gy)) \/ 
              is_corrupt tr (long_signature_key_label alice)  \/ is_corrupt tr (ephemeral_dh_key_label bob bob_si));

            // Proof strategy: We want to work without the corruption case
            // so we introduce this implication.
            let alice_not_corrupt = ~(is_corrupt tr (long_signature_key_label alice)) in
            ...
            assert(is_corrupt tr (long_signature_key_label alice) \/

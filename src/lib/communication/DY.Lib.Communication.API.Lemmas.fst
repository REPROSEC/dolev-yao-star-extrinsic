...
let receive_authenticated_proof #invs tr higher_layer_preds comm_keys_ids receiver msg_id =
  ...
  | (Some cm', tr_out) -> (
    ...
    verify_message_proof tr sender receiver msg_signed vk_sender;
    let Some cm = verify_message receiver msg_signed vk_sender in
    let ((), tr) = trigger_event receiver (CommAuthReceiveMsg cm.sender receiver cm.payload) tr in
    assert(event_triggered tr receiver (CommAuthReceiveMsg cm.sender receiver cm.payload));
    assert(tr_out == tr);
    ()
  )

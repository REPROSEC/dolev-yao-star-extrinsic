...
let comm_client_state_invariant #cinvs tr a #ps prin req_meta_data =
  comm_meta_data_knowable tr a prin req_meta_data /\
  event_triggered tr prin (CommClientSendRequest (Some? req_meta_data.client) prin req_meta_data.server req_meta_data.request req_meta_data.key <: communication_reqres_event a)
...
val send_request_proof:
  ...
  (ensures (
    ...
    | (Some (_, req_meta_data), tr_out) -> (
      trace_invariant tr_out /\
      event_triggered tr_out client (CommClientSendRequest false client server request req_meta_data.key <: communication_reqres_event a) /\
...
let send_request_proof #invs #a #config #crpreds tr comm_keys_ids  client server request =
  ...
  | (None, tr_out) -> (
    ...
    let ((), tr') = trigger_event client (CommClientSendRequest false client server request key <: communication_reqres_event a) tr' in
...
val receive_request_proof:
  ...
  (ensures (
    ...
    | (Some (payload, req_meta_data), tr_out) -> (
      trace_invariant tr_out /\
      event_triggered tr_out server (CommServerReceiveRequest None server payload req_meta_data.key <: communication_reqres_event a) /\
...
let receive_request_proof #invs a #config #crpreds tr comm_keys_ids server msg_id =
  ...
  | (Some (payload, req_meta_data), tr_out) -> (
    ...
    let req_send_event client:communication_reqres_event a = CommClientSendRequest false client server request req_msg.key in
    ...
    let ((), tr_ev) = trigger_event server (CommServerReceiveRequest None server request req_msg.key <: communication_reqres_event a) tr_recv in
...
val decode_response_proof:
  ...
  (requires
    ...
    is_secret (comm_label client server) tr req_meta_data.key /\
    req_meta_data.key `has_usage tr` (AeadKey (comm_layer_aead_tag a) empty)
  )
  (ensures (
    match decode_response_message server req_meta_data.key response_bytes req_meta_data with
    | None -> True
    | Some response -> (
      is_well_formed a (is_knowable_by (get_label tr req_meta_data.key) tr) response /\
      is_knowable_by (get_label tr req_meta_data.key) tr (serialize a response) /\
      ((exists request. event_triggered tr server (CommServerSendResponse req_meta_data.client server request response req_meta_data.key <: communication_reqres_event a))
        \/ is_corrupt tr (principal_label client) \/ is_corrupt tr (principal_label server))
    )
  ))
let decode_response_proof #invs #a tr client server response_bytes req_meta_data =
  reveal_opaque (`%decode_response_message) (decode_response_message #a);
  match decode_response_message #a server req_meta_data.key response_bytes req_meta_data with
  | None -> ()
  | Some response -> (
    ...
    serialize_wf_lemma authenticated_data (is_publishable tr) {client=req_meta_data.client; server};
    let ad_bytes = serialize authenticated_data {client=req_meta_data.client; server} in
...
val request_response_property:
  ...
  (requires
    trace_invariant tr /\
    has_communication_layer_reqres_predicates a /\
    event_triggered tr client (CommClientSendRequest (Some? req_meta_data.client) client server request req_meta_data.key <: communication_reqres_event a) /\
    is_secret (comm_label client server) tr req_meta_data.key /\
    req_meta_data.key `has_usage tr` (AeadKey (comm_layer_aead_tag a) empty) /\
    (match req_meta_data.client with
    | None -> true
  ...
  (ensures
    (
      event_triggered tr server (CommServerSendResponse req_meta_data.client server request response req_meta_data.key <: communication_reqres_event a) /\
      event_triggered tr server (CommServerReceiveRequest req_meta_data.client server request req_meta_data.key <: communication_reqres_event a)
    ) \/ is_corrupt tr (principal_label client) \/ is_corrupt tr (principal_label server)
  )
let request_response_property #invs #a #config #crpreds tr client server request response req_meta_data =
  let send_event request:communication_reqres_event a = CommServerSendResponse req_meta_data.client server request response req_meta_data.key in
  introduce (~(is_corrupt tr (principal_label client) \/ is_corrupt tr (principal_label server))) ==> 
    (exists request'. event_triggered #(communication_reqres_event a) #(event_communication_reqres_event #a #config) tr server (CommServerSendResponse req_meta_data.client server request' response req_meta_data.key <: communication_reqres_event a)) 
  with _. (
    eliminate exists request'. event_triggered tr server (CommServerSendResponse req_meta_data.client server request' response req_meta_data.key <: communication_reqres_event a)
    returns event_triggered tr server (CommServerSendResponse req_meta_data.client server request response req_meta_data.key <: communication_reqres_event a) /\
            event_triggered tr server (CommServerReceiveRequest req_meta_data.client server request req_meta_data.key <: communication_reqres_event a)
    with _. (
      ...
      assert(event_triggered tr server (CommServerReceiveRequest req_meta_data.client server request' req_meta_data.key <: communication_reqres_event a));
      match req_meta_data.client with
      | None -> (
        assert((exists client'. 
          event_triggered tr client' (CommClientSendRequest (Some? req_meta_data.client) client' server request' req_meta_data.key <: communication_reqres_event a)));
        eliminate exists client'. event_triggered tr client' (CommClientSendRequest (Some? req_meta_data.client) client' server request' req_meta_data.key <: communication_reqres_event a)
        returns event_triggered tr server (CommServerSendResponse req_meta_data.client server request response req_meta_data.key <: communication_reqres_event a) /\
                event_triggered tr server (CommServerReceiveRequest req_meta_data.client server request req_meta_data.key <: communication_reqres_event a)
        with _. (
          comm_client_send_request_injective tr (Some? req_meta_data.client) client client' server request request' req_meta_data.key;
          ()
        )
      )
      | Some client'' -> (
        ...
        assert(
          event_triggered tr client (CommClientSendRequest true client server request' req_meta_data.key <: communication_reqres_event a) \/
          is_corrupt tr (long_term_key_label client)
        );
        //assert((principal_label client) `can_flow tr` (long_term_key_label client));
        eliminate  event_triggered tr client (CommClientSendRequest true client server request' req_meta_data.key <: communication_reqres_event a) \/ is_corrupt tr (long_term_key_label client)
        returns _
        with _. (
          comm_client_send_request_injective tr true client client server request request' req_meta_data.key;
          ()
        )
        and _. (
          assert((principal_label client) `can_flow tr` (long_term_key_label client));
...
val receive_response_proof:
  ...
  (requires
    ...
    event_triggered tr client (CommClientSendRequest (Some? req_meta_data.client) client req_meta_data.server req_meta_data.request req_meta_data.key <: communication_reqres_event a)
  )
  (ensures (
    ...
    | (Some (payload, _), tr_out) -> (
      trace_invariant tr_out /\
      event_triggered tr_out client (CommClientReceiveResponse (Some? req_meta_data.client) client req_meta_data.server req_meta_data.request payload req_meta_data.key <: communication_reqres_event a)
    )
  ))
  [SMTPat (trace_invariant tr);
  SMTPat (reqres_comm_layer_lemmas_enabled a);
...
let receive_response_proof #invs #a #crpreds tr client req_meta_data msg_id =
  ...
  | (Some (response, _), tr_out) -> (
    ...
    decode_response_proof #invs.crypto_invs #a tr' client server resp_msg_bytes req_meta_data;
    let Some response = decode_response_message server key resp_msg_bytes req_meta_data in
    request_response_property tr' client server req_meta_data.request response req_meta_data;
    assert(
      match req_meta_data.client with
    | None -> true
    | Some client' ->  client = client'
    );
    //assert(req_meta_data.client == (if (Some? req_meta_data.client) then Some client else None));
    //admit();
    assert(
      event_triggered tr' server (CommServerSendResponse (if (Some? req_meta_data.client) then Some client else None) server req_meta_data.request response key <: communication_reqres_event a) \/
        is_corrupt tr' (principal_label client) \/ is_corrupt tr' (principal_label server)
    );
    //admit();
    //get_response_label_eq_key_label tr' req_meta_data;
    let ((), tr') = set_state client req_meta_data.sid (ClientReceiveResponse {server; response; key} <: communication_states a) tr' in
    assert(trace_invariant tr');
    //admit();
    let ((), tr') = trigger_event client (CommClientReceiveResponse (Some? req_meta_data.client) client server req_meta_data.request response key <: communication_reqres_event a) tr' in
    assert(event_triggered tr' client (CommClientSendRequest (Some? req_meta_data.client) client server req_meta_data.request key <: communication_reqres_event a));
    //admit();
    assert((event_predicate_communication_layer_reqres a) tr' client (CommClientReceiveResponse (Some? req_meta_data.client) client server req_meta_data.request response key <: communication_reqres_event a)) by (
      let open FStar.Tactics in
      norm [delta_only [`%event_predicate_communication_layer_reqres]; iota];
      dump "";
      //admit_all ();
